...
class DbHelper {
  ...
  static const _dbName = 'ayoluchun.db';
  ...
  Future<List<Map<String, dynamic>>> getLastRaw(
      String table, String column) async {
    final db = await instance.database;
    return await db
        .rawQuery('SELECT * FROM $table ORDER BY $column DESC LIMIT 1');
  }
  ...
  Future<List<Map<String, dynamic>>> queryByForeignKey(
      String table, int key, String where) async {
    final db = await instance.database;
  ...
  Future updateByOfflineId(String table, Map<String, dynamic> row) async {
    ...
    return await db
        .update(table, row, where: 'offline_id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> query(
      String table, String where, dynamic key) async {
    final db = await instance.database;

    return await db.query(table, where: '$where = ?', whereArgs: [key]);
  }

  Future<List<Map<String, dynamic>>> queryForNull(
  ...
  Future<List<Map<String, dynamic>>> queryWithOp(
      String table, String where, String op, dynamic key) async {
    final db = await instance.database;
